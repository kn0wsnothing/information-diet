...
import { parseStoryGraphCSV, normalizeStoryGraphBook } from "@/lib/csv-parser";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const books = parseStoryGraphCSV(text);
    ...
    for (const book of books) {
      const normalised = normalizeStoryGraphBook(book);
    ...
    console.error("StoryGraph import error:", error);
    return NextResponse.json(
      { error: "Failed to import StoryGraph data" },
      { status: 500 }
    );
  }
}
